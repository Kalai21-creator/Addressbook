...
{
    FILE *fptr = fopen("doc.txt", "r"); // Open file for reading
    ...
    fclose(fptr); // Close the file after load the contacts
}
...
{
    FILE *fptr = fopen("doc.txt", "w+"); // Open file for writing
    if (fptr == NULL)                         // Check if file opened successfully
    {
        printf("Failed to open file for writing.\n");
        return;
    }
    fprintf(fptr, "%d\n", addressBook->contactCount); // Write number of contacts count in the file
    ...
    fclose(fptr); // Close the file after save the contacts
    ...
    while (flag)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            if (!isalpha(addressBook->contacts[addressBook->contactCount].name[i]) && addressBook->contacts[addressBook->contactCount].name[i] != ' ')
            {
                ...
                break;         // Exit loop if invalid is found
            }
        }
        if (!validname)
        {
...
// Function to Search for a contact by name, mobile number, or email
void searchContact(AddressBook *addressBook)
{
    ...
    // if char, name is found to set 0
	...
	found = 0; // Reset found flag
        ...
        for (int i = 0; i < addressBook->contactCount; i++)
        {
            if (strcmp(addressBook->contacts[i].phone, searchmobile) == 0)
            {
                ...
                printf("Mobile No: %s\n", addressBook->contacts[i].phone);
                printf("Mail id: %s\n", addressBook->contacts[i].email);
                found = 1; //  to Indicate that a contact was found
                break;
            }
        }
        if (!found)
...
// Function to edit the contact details in AddrressBook
...
void editContact(AddressBook *addressBook)
{
	...
	// Validate the provided name (alphabetic characters only)
        ...
        break;
    case 2: 
	// Editing the contact's mobile number
	...
	// Prompt for the new mobile number
        ...
        else
        {
            for (int i = 0; i < length; i++)
            {
                if (!isdigit(mobile[i]))// Check if each character is a digit or not
                {
                    valid = 0;
                    break;
                }
	...
	// Editing the contact's email address
...
// Function to Deletecontact in AddressBook
void deleteContact(AddressBook *addressBook)
{
    ...
    {
    case 1: 
	    // Case to delete a contact by name
